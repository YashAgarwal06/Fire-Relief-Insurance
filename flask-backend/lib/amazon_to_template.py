...
# takes in amazon output string as a csv from chatgpt and returns bytes of an xlsx
# and returns bytes
def process_csv_to_xlsx(amazon_output: str) -> bytes:
    # Hardcoded path of our template file
    df_format_path = os.path.join(Path(__file__).resolve().parent, "PPCW-California.xlsm")

    # Load data
    df_format = pd.read_excel(df_format_path)
    output = pd.read_csv(io.StringIO(amazon_output))
    ...
    # return bytes of an excel file rather than saving it   
    output = io.BytesIO()
    # Write the DataFrame to the BytesIO object as an Excel file
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        df_format.to_excel(writer, index=False)
